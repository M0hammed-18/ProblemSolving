...
const hasChildrenValues = (arr, character) => {
  ...
  let item = false;
  arr.forEach((e) => {
    if (Object.values(e).some((e) => e == character)) item = true;
  });
  return item;
};
...
const totalCharacters = (arr) => {
  ...
  arr.forEach((e) => {
    if (e.name) number += 1;
	if(e.spouse) number+=1;
    number += e.children.length;
  });
  return number;
};
...
const houseSize = (arr) => {
  ...
  arr.forEach((e)=>{
	  ...
	  if(e.spouse)number+=1
	  number+=e.children.length;
	  sizes.push({house:e.house,members:number})
  })
  return sizes
};
...
const houseSurvivors = (arr) => {
  ...
  let number=0;
  if(e.name)number+=1
  if(e.spouse&&!deceasedSpouses.includes(e.spouse))number+=1
number+=e.children.length
survivors.push({house:e.house,members:number})
})
return survivors
};
